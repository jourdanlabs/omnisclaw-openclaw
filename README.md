...
## OMNISCLAW + BIFROST Fork

This JourdanLabs fork adds the OMNISCLAW delivery gate: OpenClaw's local agent runtime with BIFROST verification applied to final user-visible replies before delivery. The gate is designed to hide internal verification machinery, block unsafe unsupported claims, preserve normal low-claim conversation, and keep the assistant from presenting unverified output as fact.

Start here for the fork-specific install, status, probe, and hardening notes:

- [OMNISCLAW operating guide](docs/OMNISCLAW.md)
