...
function renderSecretRefApplyCommands(
  ...
  const render = (shell: CommandShell, indent = "") => {
    ...
    return [
      `${indent}omnisclaw secrets apply --from ${quotedPlanPath} --dry-run --allow-exec`,
      `${indent}omnisclaw secrets apply --from ${quotedPlanPath} --allow-exec`,
    ];
  };
  if (platform !== "win32") {
    return render("posix");
...
export function createPluginSecretRefSetupCli(params: PluginSecretRefSetupCliParams) {
  ...
  const runSetup = async (options: SecretRefSetupOptions): Promise<void> => {
    ...
    writeSecretRefCliLine("  omnisclaw secrets audit --check --allow-exec");
    writeSecretRefCliLine("  omnisclaw secrets reload");
  };
