...
export function renderMissingApiKeyReplyCopy(params?: {
  ...
  if (provider === "openai") {
    return '⚠️ Missing API key for provider "openai". Run `omnisclaw doctor --fix` to repair stale OpenAI model/session routes, restart the gateway if doctor asks, then try again. If doctor has nothing to repair or the error persists, re-auth with `omnisclaw models auth login --provider openai` or run `omnisclaw configure`.';
  }
...
const CONTROL_UI_LOG_HINT = "To view logs, run `omnisclaw logs --follow` in a terminal.";
