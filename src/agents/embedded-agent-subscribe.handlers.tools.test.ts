...
describe("handleToolExecutionEnd cron mutation tracking", () => {
  ...
  it.each([
    ["exec", "omnisclaw cron add --at +1h --message 'follow up' --name reminder"],
    ["exec", "npx omnisclaw cron add --at=+1h --message 'follow up'"],
    ["exec", "bunx omnisclaw cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm exec omnisclaw cron add --at +1h --message 'follow up'"],
    ["exec", "pnpm dlx omnisclaw cron add --at +1h --message 'follow up'"],
    ["exec", "npx -y omnisclaw cron add --at +1h --message 'follow up'"],
    ["exec", "bunx --bun omnisclaw cron add --at +1h --message 'follow up'"],
    ...
    ["bash", "corepack pnpm exec omnisclaw cron add --at +1h --message 'follow up'"],
    ["exec", "env OPENCLAW_PROFILE=test omnisclaw cron add --at +1h --message 'follow up'"],
    ["exec", "omnisclaw cron create --at +1h --message 'follow up'"],
    ["exec", "omnisclaw --profile work cron create --at +1h --message 'follow up'"],
    ["exec", "omnisclaw --dev cron add --at +1h --message 'follow up'"],
    ["exec", "omnisclaw --log-level debug --no-color cron add --at +1h --message 'follow up'"],
    ["exec", "omnisclaw --container helper cron add --at +1h --message 'follow up'"],
    ["exec", "omnisclaw cron add --at +1h --message 'follow up || wait'"],
    ["exec", "omnisclaw cron add --at +1h --message 'follow up' 2>&1"],
  ...
  it("does not increment successfulCronAdds when shell cron add fails", async () => {
    ...
    await executeTool(ctx, {
      ...
      args: {
        command: "omnisclaw cron add --at +1h --message 'follow up' --name reminder",
      },
      isError: false,
      result: {
        details: {
  ...
  it.each([
    ["omnisclaw cron list --json", "a different cron action"],
    ["echo omnisclaw cron add --at +1h", "a command that only mentions cron add"],
    ["omnisclaw cron add --at '+1h", "an unterminated shell argument"],
    ["cd /tmp && omnisclaw cron add --at +1h", "a compound command"],
    ["omnisclaw cron add --help", "the add command help"],
    ["omnisclaw cron create -h", "the create alias help"],
    ["omnisclaw cron add --bad||true", "a masked cron failure"],
    ["omnisclaw cron add --at +1h; true", "a semicolon suffix"],
    ["omnisclaw cron add --at +1h | cat", "a pipeline suffix"],
    ["omnisclaw cron add --at +1h & true", "a background suffix"],
    ["omnisclaw cron add --at +1h\ntrue", "a newline-separated suffix"],
    ["omnisclaw cron add --bad # ignored\ntrue", "a comment-masked cron failure"],
    ["npx -y echo omnisclaw cron add --at +1h", "a package runner for another executable"],
    ["pnpm omnisclaw cron add --at +1h", "a bare pnpm package script"],
    ["corepack pnpm omnisclaw cron add --at +1h", "a corepack pnpm package script"],
    ...
    ["omnisclaw cron add --bad &>/tmp/cron.log", "a bash-only combined redirection"],
    ["omnisclaw cron add --bad &>>/tmp/cron.log", "a bash-only append redirection"],
...
describe("handleToolExecutionEnd exec approval prompts", () => {
  ...
  it("emits a deterministic unavailable payload when the initiating surface cannot approve", async () => {
    ...
    expect(text).toContain(
      "Inspect the node's effective exec policy with `omnisclaw approvals get --node node-mac-1`.",
    );
