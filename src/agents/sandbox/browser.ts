      ...
      if (isHot) {
        const hint = (() => {
          if (params.cfg.scope === "session") {
            return `omnisclaw sandbox recreate --browser --session ${params.scopeKey}`;
          }
          if (params.cfg.scope === "agent") {
            const agentId = resolveSandboxAgentId(params.scopeKey) ?? "main";
            return `omnisclaw sandbox recreate --browser --agent ${agentId}`;
          }
          return "omnisclaw sandbox recreate --browser --all";
        })();
        defaultRuntime.log(
