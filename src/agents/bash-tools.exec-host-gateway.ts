...
export async function processGatewayAllowlist(
  ...
  if (hostSecurity === "allowlist" && !allowlistSatisfied && obsoleteGeneratedApprovalCount > 0) {
    params.warnings.push(
      `${obsoleteGeneratedApprovalCount} older generated exec ${obsoleteGeneratedApprovalCount === 1 ? "approval is" : "approvals are"} inactive because they are not tied to a working directory. Run "omnisclaw doctor --fix", then rerun the workflow and choose "Always allow here".`,
    );
  }
