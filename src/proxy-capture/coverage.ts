...
export function maybeWarnAboutDebugProxyCoverage(
  ...
  warn(
    `[omnisclaw proxy] debug proxy coverage: ${summary.captured}/${summary.total} captured, ${summary.proxyOnly} proxy-only, ${summary.uncovered} uncovered.`,
  );
  warn(
    `[omnisclaw proxy] remaining gaps: ${partial.map((entry) => entry.id).join(", ")}. Run \`omnisclaw proxy coverage\` for details.`,
  );
}
