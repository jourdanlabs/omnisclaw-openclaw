  ...
  params.respond(
    ...
    errorShape(
      ...
      `config.set would remove existing agent entries: ${droppedIds.join(", ")}. ` +
        "Use the agents.delete RPC or `omnisclaw agents delete <id>` for intentional deletion.",
    ),
  );
  return true;
}
  ...
  "config.patch": async ({ params, respond, client, context }) => {
    ...
    if (!snapshot.valid) {
      respond(
        ...
        errorShape(
          ErrorCodes.INVALID_REQUEST,
          `${summarizeConfigValidationIssues(snapshot.issues)}; fix (omnisclaw doctor) before patching`,
