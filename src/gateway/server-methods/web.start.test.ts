  ...
  it("surfaces the missing official external plugin hint when no web-login provider is loaded", async () => {
    ...
    mocks.resolveMissingOfficialExternalChannelPluginRepairHints.mockReturnValue([
      {
        ...
        installCommand: "omnisclaw plugins install clawhub:@openclaw/whatsapp",
        doctorFixCommand: "omnisclaw doctor --fix",
        repairHint:
          "Install the official external plugin with: omnisclaw plugins install clawhub:@openclaw/whatsapp, or run: omnisclaw doctor --fix.",
      },
    ]);
    const respond = vi.fn();
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        ...
        message:
          "web login provider is not available. Install the official external plugin with: omnisclaw plugins install clawhub:@openclaw/whatsapp, or run: omnisclaw doctor --fix.",
      }),
    );
  ...
  it("joins multiple missing official external plugin hints when more than one configured channel is missing", async () => {
              ...
              ? [
                  {
                    ...
                    installCommand: "omnisclaw plugins install clawhub:@openclaw/signal",
                    doctorFixCommand: "omnisclaw doctor --fix",
                    repairHint:
                      "Install the official external plugin with: omnisclaw plugins install clawhub:@openclaw/signal, or run: omnisclaw doctor --fix.",
                  },
                ]
              : [],
        ),
    ...
    expect(respond).toHaveBeenCalledWith(
      ...
      expect.objectContaining({
        code: "INVALID_REQUEST",
        message:
          "web login provider is not available. Configured official external channel plugins are missing for WhatsApp, Signal. Install them with: omnisclaw plugins install clawhub:@openclaw/whatsapp; omnisclaw plugins install clawhub:@openclaw/signal, or run: omnisclaw doctor --fix.",
      }),
    );
  });
