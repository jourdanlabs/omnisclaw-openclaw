...
function formatSuppressedAmbientChannelsStartupWarning(channelIds: readonly string[]): string {
  ...
  return (
    ...
    "Configure channels.<id> (omnisclaw channels add <id>) to enable the channel, or pass " +
...
function formatConfiguredChannelMissingOwnerStartupWarning(entry: {
  ...
  return (
    ...
    `is installed or loadable (${reasons}). Run \`omnisclaw doctor --fix\` or install the ` +
