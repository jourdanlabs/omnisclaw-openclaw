...
function unavailable(reason: string): string {
  return [
    ...
    "Run `omnisclaw doctor --fix` on the machine running OpenClaw, then verify the configuration before continuing.",
  ].join("\n");
}
...
export async function verifyConfigAfterSystemAgentWrite(
  ...
  try {
    ...
    return `${notice}\nThe write was applied, but inference could not propose a repair. Run \`omnisclaw doctor --fix\` on the machine running OpenClaw, then try again.`;
  }
  return recovery.text
    ? `${notice}\n\n${recovery.text}`
    : `${notice}\nUse \`config schema <path>\` here to check the expected shape. Or, with OpenClaw stopped, run \`omnisclaw doctor --fix\` on the machine running it.`;
}
