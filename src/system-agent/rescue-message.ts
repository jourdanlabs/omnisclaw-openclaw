...
function formatUnsupportedRemoteOperation(operation: SystemAgentOperation): string | null {
  ...
  if (operation.kind === "channel-setup") {
    return [
      ...
      "Run `omnisclaw setup` locally and say `connect " + operation.channel + "` instead.",
    ].join(" ");
  }
  if (operation.kind === "model-setup") {
    return [
      "OpenClaw rescue cannot host model-provider credential setup from a message channel.",
      "Run `omnisclaw onboard` locally; it live-tests the candidate route before saving it.",
    ].join(" ");
  }
  if (operation.kind === "doctor-fix") {
    return [
      ...
      "On the machine running OpenClaw, with OpenClaw stopped, run `omnisclaw doctor --fix`.",
    ].join(" ");
  }
  if (operation.kind === "plugin-install") {
    return [
      ...
      "Use local `omnisclaw setup` or `omnisclaw plugins install` instead.",
    ].join(" ");
  }
  return null;
}
