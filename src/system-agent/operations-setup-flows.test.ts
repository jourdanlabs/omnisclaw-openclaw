...
describe("system agent setup-flow operations", () => {
  ...
  it("prints one-shot setup pointers", async () => {
    ...
    expect(output).toContain("omnisclaw onboard`");
    expect(output).toContain("omnisclaw onboard --classic");
    expect(output).toContain("omnisclaw channels add --channel slack");
    expect(output).toContain("omnisclaw configure --section web");
    expect(output).toContain("omnisclaw configure --section gateway");
  ...
  it("prints one-shot pointers for hosted skills, search, and Gateway setup", async () => {
    ...
    expect(lines.join("\n")).toContain("omnisclaw configure --section skills");
    expect(lines.join("\n")).toContain("omnisclaw configure --section web");
    expect(lines.join("\n")).toContain("omnisclaw configure --section gateway");
  });
  ...
  it("routes one-shot model setup through the verified OpenClaw flow", async () => {
    ...
    expect(result.applied).toBe(false);
    expect(lines.join("\n")).toContain("Run `omnisclaw onboard` on the machine running OpenClaw");
    expect(lines.join("\n")).toContain("Stop the OpenClaw host");
    expect(lines.join("\n")).toContain("restart the host");
    expect(lines.join("\n")).not.toContain("omnisclaw configure --section model");
  });
