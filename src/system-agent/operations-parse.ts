...
export function describeSystemAgentPersistentOperation(operation: SystemAgentOperation): string {
  switch (operation.kind) {
    ...
    case "doctor-fix":
      return "run omnisclaw doctor --fix on the machine running OpenClaw, with OpenClaw stopped";
    case "plugin-install":
...
export const SYSTEM_AGENT_OPERATOR_APPROVAL_HANDOFF =
  "This change needs operator approval and cannot be applied from this chat. Approve it in the OpenClaw operator UI (`omnisclaw dashboard` on the Gateway host), or run the change there with `omnisclaw setup`.";

export const SYSTEM_AGENT_OPERATOR_NAVIGATION_HANDOFF =
  "Channel, model, and setup flows need a human operator in the OpenClaw app; they cannot run from a delegated agent request. Open `omnisclaw dashboard` or run `omnisclaw setup` on the Gateway host.";
