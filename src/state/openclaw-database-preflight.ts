...
export function assertOpenClawDatabasesReady(
  ...
  throw new Error(
    `${action} because persisted database readiness could not be verified: ${shown.join("; ")}${omitted > 0 ? `; +${omitted} more` : ""}. Stop the Gateway and other OpenClaw processes, run omnisclaw doctor --fix, then retry.`,
  );
}
