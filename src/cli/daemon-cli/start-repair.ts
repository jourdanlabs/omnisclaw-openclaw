...
function assertGatewayRepairTargetMatches(params: {
  ...
  if (!installedStateOverride && !installedHome) {
    throw new Error(
      `Refusing to repair the managed Gateway service because its installed state directory cannot be determined from the service definition. Run \`omnisclaw gateway install --force\` to replace it intentionally.`,
    );
  }
  ...
  const details = differences
    ...
    .join("\n");
  throw new Error(
    `Refusing to repair the managed Gateway service because the current invocation targets a different Gateway:\n${details}\nRun \`omnisclaw gateway ${params.action}\` with the installed state directory, config path, and port (or unset conflicting environment overrides). To retarget intentionally, run \`omnisclaw gateway install --force\`.`,
  );
}
  ...
  return {
    ...
    message:
      params.action === "restart"
        ...
        : "Gateway service definition repaired and started. Reopen the Control UI with `omnisclaw dashboard` or copy a fresh auth URL with `omnisclaw dashboard --no-open`.",
