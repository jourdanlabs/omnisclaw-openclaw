...
describe("gatherDaemonStatus", () => {
  ...
  it("renders Gateway-specific recovery in text and JSON after service reads time out", async () => {
    ...
    expect(status.service.runtime).toEqual({
      status: "unknown",
      detail: "service runtime inspection failed; retry with omnisclaw gateway status --deep",
      inspectionFailure: {
    ...
    try {
      ...
      expect(output).toContain(
        "Runtime: unknown (service runtime inspection failed; retry with omnisclaw gateway status --deep)",
      );
      ...
      new Map([
        [
          19001,
          {
            port: 19001,
            status: "busy",
            listeners: [{ pid: 8000, ppid: 1, commandLine: "omnisclaw gateway" }],
            hints: [],
          },
        ],
      ]),
