...
function createConfigureInteractiveResult(options?: {
  ...
  return {
    plan: {
      ...
      protocolVersion: 1,
      generatedAt: "2026-02-26T00:00:00.000Z",
      generatedBy: "omnisclaw secrets configure",
...
describe("secrets CLI", () => {
  ...
  it("explains Gateway reload failures without duplicate doctor noise", async () => {
    callGatewayFromCli.mockRejectedValue(
      new Error(
        "gateway closed (1006 abnormal closure). Gateway target: ws://127.0.0.1:18789 Source: local loopback Config: /tmp/openclaw.json Bind: loopback Possible causes: - Gateway not yet ready. Run `omnisclaw doctor` for diagnostics.",
      ),
    );
    ...
    expect(runtimeErrors.at(-1)).toContain("omnisclaw gateway status --deep");
