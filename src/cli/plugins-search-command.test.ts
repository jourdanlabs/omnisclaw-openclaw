...
describe("plugins search command", () => {
  ...
  it.each([
    {
      ...
      profile: undefined,
      container: undefined,
      command: "omnisclaw plugins install clawhub:openclaw-calendar",
    },
    {
      context: "profile",
      profile: "work",
      container: undefined,
      command: "omnisclaw --profile work plugins install clawhub:openclaw-calendar",
    },
    {
      context: "container",
      profile: undefined,
      container: "staging",
      command: "omnisclaw --container staging plugins install clawhub:openclaw-calendar",
    },
    {
  ...
  it("leaves missing-query JSON failures to the root renderer", async () => {
    await expect(runPluginsSearchCommand([], { json: true }, mocks.runtime)).rejects.toThrow(
      "Usage: omnisclaw plugins search <query>",
    );
