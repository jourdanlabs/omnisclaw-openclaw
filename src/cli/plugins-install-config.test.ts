...
describe("loadConfigForInstall", () => {
      ...
      makeSnapshot({
        ...
        issues: [
          ...
          },
          {
            ...
            message:
              'web_search provider is not available: brave (install or enable plugin "brave", then run omnisclaw doctor --fix)',
          },
        ],
      }),
    );
  ...
  it("rejects non-Discord install requests when config is invalid", async () => {
    ...
    ).rejects.toThrow("Config invalid; run `omnisclaw doctor --fix` before installing plugins.");
  });
