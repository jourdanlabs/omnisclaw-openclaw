...
function formatPressureNextStep(
  ...
  return pressure.level === "critical"
    ? "nextStep=inspect latest stability bundle or run omnisclaw gateway diagnostics export; restart gateway if process is unstable"
    : "nextStep=run omnisclaw gateway status --deep and omnisclaw gateway diagnostics export; restart gateway if pressure persists";
}
