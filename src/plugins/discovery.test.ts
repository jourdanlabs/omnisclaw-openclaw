...
describe("discoverOpenClawPlugins", () => {
  ...
  it("still requires compiled runtime output for tracked installed package plugins", async () => {
    ...
    expect(
      result.diagnostics.some(
        (entry) =>
          entry.pluginId === "source-only-pack" && entry.message.includes("omnisclaw doctor --fix"),
      ),
    ).toBe(false);
  ...
  it("warns on legacy npm declaration stubs without loading workspace node_modules", async () => {
    ...
    expectDiagnostic({
      ...
      messageIncludes: 'run "omnisclaw doctor --fix"',
    });
  });
  ...
  it("skips incompatible non-bundled package plugin API candidates during discovery", () => {
    ...
    expectDiagnostic({
      ...
      messageIncludes:
        'plugin requires plugin API >=2026.5.27-beta.2, but this host is 2026.5.27-beta.1; skipping discovery (check "omnisclaw --version", OPENCLAW_COMPATIBILITY_HOST_VERSION, or run "omnisclaw doctor")',
    });
  });
