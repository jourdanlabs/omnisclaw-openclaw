...
describe("config plugin validation", () => {
    ...
    it("still reports explicit Codex allowlist entries for custom OpenAI-compatible base URLs", () => {
      ...
      expect(res.warnings ?? []).toContainEqual(
        expect.objectContaining({
          path: "plugins.allow",
          message:
            "plugin not installed: codex — install the official external plugin with: omnisclaw plugins install @openclaw/codex",
        }),
      );
    });
  ...
  it("deduplicates catalog install hints for missing configured official external plugins", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: brave — install the official external plugin with: omnisclaw plugins install @openclaw/brave-plugin";
  ...
  it("warns instead of failing when an official external memory slot plugin is not installed", () => {
    ...
    expect(res.ok).toBe(true);
    const slotMessage =
      "plugin not installed: memory-lancedb — gateway will run without persistent memory until installed; install the official external plugin with: omnisclaw plugins install @openclaw/memory-lancedb";
    const entryMessage =
      "plugin not installed: memory-lancedb — install the official external plugin with: omnisclaw plugins install @openclaw/memory-lancedb";
  ...
  it("deduplicates yuanbao missing-plugin warnings across entries and allow", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin not installed: yuanbao — install the official external plugin with: omnisclaw plugins install openclaw-plugin-yuanbao@2.18.2";
  ...
  it("warns instead of failing for stale channel config backed by missing plugin refs", () => {
    ...
    expect(res.warnings).toContainEqual({
      ...
      message:
        "unknown channel id: missing-chat (stale channel plugin config ignored; run omnisclaw doctor --fix to remove stale config, or install the plugin)",
    });
    expect(res.warnings).toContainEqual({
      path: "plugins.allow",
      message:
    ...
    expect(res.warnings ?? []).toEqual([
      {
        path: "plugins.allow",
        message:
          "plugin not installed: discord — install the official external plugin with: omnisclaw plugins install @openclaw/discord",
      },
    ]);
  });
  ...
  it("uses persisted installed-plugin records as stale channel evidence", async () => {
    ...
    try {
      ...
      expect(res.warnings).toContainEqual({
        ...
        message:
          "unknown channel id: missing-sms (stale channel plugin config ignored; run omnisclaw doctor --fix to remove stale config, or install the plugin)",
      });
    } finally {
      await writePersistedInstalledPluginIndex(
        {
  ...
  it("warns for removed skill-workshop plugin id instead of failing validation", () => {
    ...
    expect(res.ok).toBe(true);
    const message =
      "plugin removed: skill-workshop (stale plugin config ignored; Skill Workshop is built into OpenClaw skills now. Use skills.workshop settings and omnisclaw skills workshop commands, then remove this plugins config entry)";
