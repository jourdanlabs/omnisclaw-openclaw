  ...
  const { extensionsDir, pluginDirs } = await listInstalledPluginDirs({
    ...
    onReadError: (err) => {
      findings.push({
        ...
        remediation:
          "Check file permissions and plugin layout, then rerun `omnisclaw security audit --deep`.",
      });
    },
  });
    ...
    for (const entry of entries) {
      ...
      const summary = await getSkillCodeSafetySummary({
        ...
        findings.push({
          ...
          remediation:
            "Check file permissions and skill layout, then rerun `omnisclaw security audit --deep`.",
        });
        return null;
      });
      if (!summary) {
