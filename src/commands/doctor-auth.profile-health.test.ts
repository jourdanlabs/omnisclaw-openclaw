...
describe("noteAuthProfileHealth", () => {
  ...
  it.each([
    [
      "auth_permanent",
      "Re-authenticate with `omnisclaw models auth login --provider openai --profile-id 'openai:disabled'`.",
    ],
  ...
  it.each([
    [
      ...
      "- openai-codex:default: re-auth required [refresh_token_reused] — Run `omnisclaw models auth login --provider openai`.",
    ],
    [
      "openai-codex:default",
      "OAuth token refresh failed for openai-codex: temporary upstream issue. Please try again or re-authenticate.",
      "- openai-codex:default: OAuth refresh failed — Try again; if this persists, run `omnisclaw models auth login --provider openai`.",
    ],
    [
      "OpenAI Work Profile",
      "OAuth token refresh failed for openai: invalid_grant. Please try again or re-authenticate.",
      "- OpenAI Work Profile: re-auth required [invalid_grant] — Run `omnisclaw models auth login --provider openai --profile-id 'OpenAI Work Profile'`.",
    ],
    [
      "openai-codex:default",
      "OAuth token refresh failed for openai-codex`\nrm -rf /: invalid_grant. Please try again or re-authenticate.",
      "- openai-codex:default: re-auth required [invalid_grant] — Run `omnisclaw models auth login --provider openai`.",
    ],
  ])(
