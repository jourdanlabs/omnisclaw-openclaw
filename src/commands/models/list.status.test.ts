    ...
    try {
      ...
      expect(parseFirstJsonLog(jsonRuntime).auth.unusableProfiles).toEqual([
        expect.objectContaining({
          ...
          recoveryHint:
            "Re-authenticate with `omnisclaw models auth login --provider anthropic --profile-id 'anthropic:default'`.",
        }),
      ]);
      ...
      expect(output).toContain("omnisclaw models auth login --provider anthropic");
    } finally {
