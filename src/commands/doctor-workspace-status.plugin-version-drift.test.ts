  ...
  it("reports older and newer pins as advisory drift while accepting correction suffixes", () => {
    ...
    for (const installedVersion of ["2026.5.30", "2026.6.2"]) {
      ...
      expect(collectWorkspaceStatusHealthFindings(config, { pluginVersionDrift: report })).toEqual([
        {
          ...
          fixHint: "omnisclaw plugins update @openclaw/codex@2026.6.1 && omnisclaw gateway restart",
        },
      ]);
      ...
      try {
        ...
        expect(driftNotes[0]?.[0]).toContain("omnisclaw plugins update @openclaw/codex@2026.6.1");
        expect(driftNotes[0]?.[0]).toContain("omnisclaw gateway restart");
      } finally {
        noteSpy.mockRestore();
      }
    }
