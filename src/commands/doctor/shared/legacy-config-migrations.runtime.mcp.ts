...
const MCP_SERVER_TYPE_RULE: LegacyConfigRule = {
  path: ["mcp", "servers"],
  message:
    'mcp.servers entries use OpenClaw transport names; CLI-native type aliases are legacy here. Run "omnisclaw doctor --fix".',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_DISABLED_RULES: LegacyConfigRule[] = [
  ...
  message:
    ...
    'Run "omnisclaw doctor --fix" to migrate it.',
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_TIMEOUT_ALIASES_RULES: LegacyConfigRule[] = [
  ...
  path,
  message: `${path.join(".")} timeout aliases were retired; use connectionTimeoutMs and requestTimeoutMs. Run "omnisclaw doctor --fix".`,
  match: (value) =>
    isRecord(value) &&
...
const MCP_SERVER_ALIASES_RULES: LegacyConfigRule[] = [
  ...
  path,
  message: `${path.join(".")} legacy aliases were retired; use camelCase spellings and cwd. Run "omnisclaw doctor --fix".`,
  match: (value) =>
    isRecord(value) &&
