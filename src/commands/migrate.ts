...
export async function migrateApplyCommand(
  ...
  if (!opts.yes && !process.stdin.isTTY) {
    throw new Error(
      `${formatCliCommand("openclaw migrate apply")} requires --yes in non-interactive mode. Preview first with ${formatCliCommand(`openclaw migrate plan '${providerId.replaceAll("'", "'\\''")}'`)}.`,
    );
  }
