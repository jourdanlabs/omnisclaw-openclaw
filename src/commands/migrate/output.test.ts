...
describe("formatMigrationPreview", () => {
  ...
  it("renders migration warnings with a warning glyph", () => {
    const output = formatMigrationPreview({
      ...plan([skillItem(1)]),
      warnings: [
        "Some Codex plugins could not be migrated. Run `omnisclaw migrate codex` after onboarding.",
      ],
    })
      .map(stripAnsi)
      .join("\n");
    ...
    expect(output).toContain(
      "⚠️  Some Codex plugins could not be migrated. Run `omnisclaw migrate codex` after onboarding.",
    );
  });
...
describe("formatMigrationResult", () => {
  ...
  it("renders provider warnings absent from migration next steps", () => {
    ...
    expect(output).toContain("• Run omnisclaw doctor after applying the migration.");
  });
  ...
  it("renders warning-backed next steps with a warning glyph", () => {
    ...
    const output = formatMigrationResult({
      ...
      warnings: [warning],
      nextSteps: [warning, "Run omnisclaw doctor after applying the migration."],
    })
      .map(stripAnsi)
      .join("\n");
