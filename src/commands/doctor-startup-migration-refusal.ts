...
export async function describeLiveGatewayOwnerStartupBlocker(
  ...
  return `Another gateway (pid ${activeGateway.pid}) already owns this state directory; refusing to run automatic startup migrations or report the gateway ready. Stop it with "omnisclaw gateway stop" (or select a different OPENCLAW_STATE_DIR), then retry startup.`;
}
