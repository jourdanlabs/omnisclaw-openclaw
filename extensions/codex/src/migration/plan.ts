    ...
    items.push(
      createMigrationManualItem({
        ...
        message:
          ...
          `Codex native plugin "${plugin.name}" was found but not activated automatically.`,
        recommendation:
          "Review the plugin bundle first, then install trusted compatible plugins with omnisclaw plugins install <path> --force.",
      }),
    );
  }
  return items;
...
export async function buildCodexMigrationPlan(
  ...
  return {
    ...
    nextSteps: memoryOnly
      ? []
      : [
          "Run omnisclaw doctor after applying the migration.",
