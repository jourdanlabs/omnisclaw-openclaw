...
describe("ollama web search guarded fetch", () => {
  it("cancels a stalled 401 body before guarded release", async () => {
    ...
    await expect(tool.execute({ query: "latest omnisclaw release" })).rejects.toThrow(
      "ollama signin",
    );
  ...
  it("redacts a reflected bearer credential from a real completed 429 response", async () => {
    ...
    const error = await tool
      .execute({ query: "latest omnisclaw release" })
    ...
    loopback.status = 200;
    const result = await tool.execute({ query: "latest omnisclaw release" });
