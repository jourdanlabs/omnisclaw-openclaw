    ...
    execute: async (_toolCallId, args, signal) => {
      ...
      if (action === "discover") {
        ...
        return jsonResult({
          nodes,
          ...(modelNodes.length === 0 && {
            hint: "No connected node advertises Ollama inference. Start Ollama and `omnisclaw node run` on the target machine, then approve any request shown by `omnisclaw nodes pending`.",
          }),
        });
      }
