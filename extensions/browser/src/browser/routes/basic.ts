...
async function runBrowserLiveProbe(profileCtx: ProfileContext, signal: AbortSignal) {
  ...
  try {
    ...
    return {
      ...
      summary: String(err),
      fixHint: "Run omnisclaw browser start, then retry with omnisclaw browser doctor --deep.",
    };
  }
}
...
function parseHeadlessStartOverride(params: {
  ...
  if (
    ...
    jsonError(
      params.res,
      400,
      `Headless start override is only supported for locally launched omnisclaw profiles. Profile "${params.profileCtx.profile.name}" is attach-only, remote, or existing-session.`,
    );
    return { ok: false };
  }
