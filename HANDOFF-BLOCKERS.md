...
| ------------------------- | ---------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Release validators (D)    | `package-source-preflight` · `release-plan-producer` | Fork prerelease tag `v2026.8.1-omnisclaw.0` is rejected as `invalid release tag`; `packages/ai` version-match wants `openclaw 2026.8.1-omnisclaw.0`. **Not inherit.** Teach the validators the fork tag, or exclude these upstream scripts from the fork suite with a named reason. Toph will not decide. |
...
| TERMINUS test-mode      | Production `actionGateEnabled()` stays ON outside tests. `VITEST` sniff is a **test-time bypass only** (no CADUCEUS in OpenClaw CI). Spawned children that replace `env` do not inherit `VITEST`; they inherit `OMNISCLAW_TERMINUS_ACTION=0` from `test/setup.shared.ts` and `runCliProcessChild`. Not a production hole. |

## Other (logged, not faked)

| Item                    | Fact                                                                                                                                                                                                                                                                                                                                                                   |
| ----------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Full suite re-run       | `/tmp/omnisclaw-full-test-cli-copy-guard2.log`: **597 shards / 10550.23s / 39 shards red / 60 unique files**. All 14 inherit files still red. **Not inherit-only.** Remaining split in `BUILD-RECEIPT.md` (leftover copy folded into the same commit after the run; load flakes isolation-green; tooling/version not this copy pass). **Pan re-gates. §1 not closed.** |
| `pnpm test:unit:fast`   | After last copy+guard wrap: **15387 passed / 6 skipped / 0 failed** (`/tmp/omnisclaw-unit-fast5.log`). `pnpm check:cli-copy` clean.                                                                                                                                                                                                                                    |
